...
| **APP-MUA-05** | Story | **Hồ Sơ Tay Nghề & Cài Đặt Bán Kính Hoạt Động**<br>• Chỉnh sửa Bio giới thiệu bản thân, số năm thâm niên trong nghề.<br>• Thanh trượt chọn Bán kính phục vụ tối đa (1km – 50km) để hệ thống lọc đơn phát sóng.<br>• Đăng ký danh sách các phong cách make-up sở trường. | MUA | `src/app/mua/profile-settings.tsx`<br>`src/components/mua/RadiusSliderPicker.tsx` | **Medium** | `MuaProfileController`<br>`MuaStyleController` |
| **APP-MUA-06** | Story | **Quản Lý Gói Dịch Vụ & Bộ Ảnh Mẫu Đi Kèm (Package & Media Manager)**<br>• Tạo mới / Chỉnh sửa gói dịch vụ cá nhân (Cô dâu, Dự tiệc, Kỷ yếu...).<br>• Thiết lập giá tiền niêm yết, thời lượng dự kiến (phút), các bước thực hiện.<br>• Trực tiếp quản lý **Bộ sưu tập ảnh mẫu đã make thực tế** hiển thị tương ứng khi khách bấm vào gói này.<br>• Bật / Tắt trạng thái mở nhận lịch của gói dịch vụ. | MUA | `src/app/mua/packages.tsx`<br>`src/app/mua/package-create.tsx` | **High** | `ServicePackageController`<br>`/api/v1/packages` |
