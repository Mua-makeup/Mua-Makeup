...
| **A4** | **Lịch Hẹn & Quản Lý Booking Studio**<br>`/agency/bookings` | • Bảng giám sát đơn đặt lịch của cơ sở: Mã đơn, Khách hàng, Gói dịch vụ, Thời gian, Trạng thái đơn, Thợ đảm nhiệm.<br>• Modal chi tiết lịch sử diễn tiến đơn (`/bookings/{id}/history`). | • Xem timeline chi tiết 5 chặng thực hiện của đơn hàng.<br>• Tự động cập nhật trạng thái đơn qua STOMP WebSocket. | • `GET /api/v1/agency/bookings`<br>• `GET /api/v1/bookings/{id}/history` |
...
| **B6** | **Giám Sát Toàn Bộ Đơn Hàng Booking Toàn Sàn**<br>`/admin/bookings` | • Bảng theo dõi mọi đơn đặt lịch: Mã đơn, Khách hàng, Chuyên viên MUA, Địa chỉ, Tổng tiền, Trạng thái đơn, Phương thức thanh toán.<br>• Modal xem chi tiết đơn và lịch sử diễn tiến 5 chặng. | • Giám sát trạng thái đơn hàng thời gian thực.<br>• Tra cứu nhanh đơn có tranh chấp hoặc bị khiếu nại. | • `GET /api/v1/admin/bookings`<br>• `GET /api/v1/admin/bookings/{id}`<br>• `GET /api/v1/bookings/{id}/history` |
