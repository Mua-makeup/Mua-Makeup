...
export const AdminDashboardPage = () => {
  ...
  const handleVerifySuccess = ({ muaId, isVerified }) => {
    setPendingMuas((prev) =>
      prev.map((m) =>
        m.muaId === muaId
          ? { ...m, status: isVerified ? 'VERIFIED' : 'REJECTED' }
          : m
      )
    );
    setToastMessage(
  ...
  const pendingCount = pendingMuas.filter((m) => m.status === 'PENDING').length;
    ...
    <div className="space-y-6">
      ...
      {apiError && (
        <div className="p-4 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-2xl text-rose-800 dark:text-rose-300 text-xs flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-sm">Cảnh Báo Lỗi Kết Nối Backend / CSDL:</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between transition-colors">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Từ agency_profiles</p>
          </div>
          <div className="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center border border-indigo-100 dark:border-indigo-900">
        ...
        <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between transition-colors">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Từ DB catalog_schema</p>
          </div>
        ...
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Dữ liệu trực tiếp từ mua_schema.mua_profiles (chứng chỉ tải lên)
            </p>
          </div>
          <Link
                ...
                {pendingMuas.map((mua) => (
                  <tr key={mua.muaId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button
                        variant={mua.status === 'PENDING' ? 'primary' : 'secondary'}
                        size="sm"
                        onClick={() => setSelectedCert(mua)}
                      >
                        {mua.status === 'PENDING' ? t('verify_action') : t('review_again')}
                      </Button>
                    </td>
                  </tr>
                ))}
